...
class Database(AbstractDatabase):
    ...
    async def upsert(
        ...
        """
        # upsert2:
        # - Try UPDATE first using `where` columns that have non-None values.
        ...
        # - caller can choose different columns for UPDATE matching (`where`)
        # - `keys` are treated as identity columns and are never included in UPDATE SET
        # - safer under concurrency than plain select-then-insert
        # - allows database-generated values by skipping `server_defaults`
        #   when their input value is None
        #
        # Arguments:
        # - table: target table name
        # - keys: identity/unique columns used for response shape on error;
        #   these columns are also excluded from UPDATE SET
        # - where: columns used to match existing row for UPDATE
        # - values: all column values for insert/update decision
        # - server_defaults: columns generated by DB when input value is None,
        #   so they are removed from INSERT
        #
        # Notes:
        # - only `where` columns with non-None values are used in UPDATE WHERE
        # - UPDATE SET uses columns from `values` except active `where` columns and `keys`
        # - INSERT uses all `values` except `server_defaults` with None value
...
async def upsert2(
    ...
    where: list[str],
    values: dict,
    server_defaults: list[str] | None = None,
    conn=None,
):
    ...
    # Split columns:
    # - active_where_cols: where-cols with a non-None value → go to WHERE
    # - update_cols: everything else except keys → go to SET
    # - keys: uniqueness identifiers, never go to SET
    # - server_defaults with None value: stripped from INSERT, let DB generate
    active_where_cols = [
        k for k in where if k in values and values[k] is not None
    ]

    update_cols = [
        k for k in values.keys() if k not in active_where_cols and k not in keys
    ]
    ...
    try:
        async with self._get_connection(conn) as conn:
            async with conn.transaction():
                ...
                # 1) Try UPDATE first
                # ------------------------------------------------
                if active_where_cols and update_cols:
                ...
                # ------------------------------------------------
                if row is None:
                    # Strip server_defaults columns where value is None — let DB generate
                    insert_data = {
                        k: v
                        for k, v in values.items()
                        if not (k in server_defaults and v is None)
                    }
                    ...
                    try:
                        ...
                        #    retry the UPDATE
                        # ----------------------------------------
                        if not (active_where_cols and update_cols):
                            raise
        ...
        # Standalone call: soft error with keys set to None
        error_fields = {k: None for k in keys}
